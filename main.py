...
COUNTRY = st.secrets['country']

st.set_page_config(page_title=F"{COUNTRY} {LEAGUE} Overview")

teams = list(collection.find({'general.league': LEAGUE, 'general.season': YEAR, 'general.country': COUNTRY}).distinct('teams.home.name'))
...
with st.sidebar:
    ...
    st.write(f"This App was designed in order to get an overview of {GENTILIC} {LEAGUE} regarding xG Open Play Per 100 Passes")
...
#get data
df = get_data(league=LEAGUE, season=YEAR, country=COUNTRY, teams=teams)
